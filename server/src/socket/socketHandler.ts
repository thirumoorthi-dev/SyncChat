...
export function setupSocket(io: Server) {
  ...
  io.on('connection', async (socket: AuthenticatedSocket) => {
    try {
      ...
      await MessageModel.markAllDirectMessagesAsDelivered(userId);

      socket.broadcast.emit('user_online', { userId, isOnline: true });

      socket.broadcast.emit('messages_delivered', { byUserId: userId });
      ...
      socket.on('send_message', async (data, callback) => {
        ...
        try {
          ...
          if (receiverSocketId) {
            // [Phase 1] Mark as delivered immediately
            await MessageModel.markMessagesAsDelivered(inserted.rows[0].id);
            fullMessage.is_delivered = true;
            io.to(receiverSocketId).emit('new_message', fullMessage);

            // Notify sender it was delivered
            socket.emit('message_delivered', { messageId: fullMessage.id, receiverId });
          } else {
            // [Phase 2] Send push notification if offline
            sendPushNotification(receiverId, {
              title: fullMessage.sender_username || 'New Message',
              body: fullMessage.content || `Sent a ${fullMessage.message_type}`,
              icon: '/logo.png',
              data: { chatId: userId, type: 'direct' }
            });
          }
      ...
      socket.on('send_group_message', async (data, callback) => {
        ...
        try {
          ...
          // [Phase 2] Send push notifications to group members (who are not the sender)
          const members = await GroupModel.getGroupMembers(groupId);
          members.rows.forEach(m => {
            if (m.user_id !== userId && !onlineUsers.has(m.user_id)) {
              sendPushNotification(m.user_id, {
                title: `${fullMessage.sender_username || 'Someone'} in ${fullMessage.group_name || 'Group'}`,
                body: fullMessage.content || `Sent a ${fullMessage.message_type}`,
                icon: '/logo.png',
                data: { chatId: groupId, type: 'group' }
              });
            }
          });
      ...
      socket.on('call_user', ({ to, offer, type }) => {
        ...
        if (targetSocketId) {
          io.to(targetSocketId).emit('call_received', { from: userId, fromName: socket.user!.username, offer, type });
        } else {
      ...
      socket.on('reject_call', ({ to }) => {
        const targetSocketId = onlineUsers.get(to);
        if (targetSocketId) {
          io.to(targetSocketId).emit('call_rejected', { from: userId });
        }
      });
      ...
      socket.on('end_call', ({ to }) => {
        const targetSocketId = onlineUsers.get(to);
        if (targetSocketId) {
          io.to(targetSocketId).emit('call_ended', { from: userId });
        }
      });
    } catch (err) {
