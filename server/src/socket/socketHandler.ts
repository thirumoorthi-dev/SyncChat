...
export function setupSocket(io: Server) {
  ...
  io.on('connection', async (socket: AuthenticatedSocket) => {
    ...
    socket.on('send_message', async (data, callback) => {
      ...
      try {
        const inserted = await MessageModel.sendDirectMessage(userId, parseInt(receiverId), content.trim());
    ...
    socket.on('send_group_message', async (data, callback) => {
      ...
      try {
        // Verify membership
        const member = await GroupModel.checkMembership(parseInt(groupId), userId);
        ...
        const inserted = await MessageModel.sendGroupMessage(userId, parseInt(groupId), content.trim());
    ...
    socket.on('typing_start', ({ receiverId, groupId }) => {
      ...
      if (groupId) {
        socket.to(`group_${groupId}`).emit('typing_start', {
          userId, username: socket.user.username, groupId: parseInt(groupId)
        });
      }
    });
    ...
    socket.on('typing_stop', ({ receiverId, groupId }) => {
      ...
      if (groupId) {
        socket.to(`group_${groupId}`).emit('typing_stop', { userId, groupId: parseInt(groupId) });
      }
    });
    ...
    socket.on('mark_read', async ({ senderId }) => {
      if (!senderId) return;
      try {
        await MessageModel.markDirectMessagesAsRead(userId, parseInt(senderId));
    ...
    socket.on('mark_group_read', async ({ groupId }) => {
      ...
      try {
        ...
        await MessageModel.markGroupMessagesRead(parseInt(groupId), userId);

        // Notify the sender's own socket so sidebar clears the unread badge
        socket.emit('group_messages_read', { groupId: parseInt(groupId), byUserId: userId });
      } catch (err) {
