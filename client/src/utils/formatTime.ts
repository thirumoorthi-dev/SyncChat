...
export function formatConversationTime(date: string | number | Date | null | undefined): string {
  if (!date) return '';
  const d = new Date(date);
...
export function formatLastSeen(date: string | number | Date | null | undefined, isOnline: boolean): string {
