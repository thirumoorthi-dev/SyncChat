...
export default function NewChatModal({ onClose, onSelectUser, onCreateGroup }: NewChatModalProps) {
  ...
  const handleAddContactAndChat = async () => {
    if (!searchResult) return;
    if (searchResult.is_contact) {
  ...
  const handleCreateGroup = () => {
    ...
    onCreateGroup(groupName.trim(), selectedUsers.map(u => parseInt(u.id, 10) || u.id as any));
  };
      ...
      >
        ...
        <div className="flex flex-shrink-0" style={{ borderBottom: '1px solid var(--border)' }}>
          {(['direct', 'group'] as const).map(t => (
            <button
              key={t}
              onClick={() => { setTab(t); setSearch(''); setSearchResult(null); setSearchError(''); }}
              className={`flex-1 py-3 text-sm font-medium transition-colors relative ${
                tab === t ? '' : 'hover:bg-[var(--hover)]'
              }`}
              style={{ color: tab === t ? 'var(--teal)' : 'var(--subtext)' }}
            >
              {t === 'direct' ? 'Add Contact' : 'New Group'}
              {tab === t && (
                <span className="absolute bottom-0 left-0 right-0 h-0.5 rounded-full" style={{ background: 'var(--teal)' }} />
              )}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
          {tab === 'group' && (
            <input
              ...
              style={{ background: 'var(--input-bg)', color: 'var(--text)', border: `1.5px solid ${groupName ? 'var(--teal)' : 'var(--border)'}` }}
            />
          )}
          ...
          {tab === 'direct' ? (
            <form onSubmit={handleSearchEmail} className="flex gap-2">
              ...
              <button
                ...
                className="btn-primary rounded-xl px-4 text-sm font-semibold disabled:opacity-50"
              >
            ...
            <div className="relative">
              <svg className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color: 'var(--subtext)' }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="flex flex-wrap gap-2">
              {selectedUsers.map(u => (
                ...
                >
                  ...
                  {u.username}
                  <button onClick={() => toggleUser(u)} className="ml-0.5 hover:text-red-400 transition-colors font-bold">×</button>
                </span>
              ))}
            </div>
          )}
          ...
          <div className="flex flex-col gap-0.5 mt-2">
            {tab === 'direct' ? (
               <>
                 {searchError && (
                   <div className="p-3 text-sm rounded-xl text-red-500 bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/30">
                     {searchError}
                   </div>
                 )}
                 {searchResult && (
                   <div className="flex items-center gap-3 px-3 py-3 rounded-xl border border-[var(--border)] bg-[var(--hover)] mt-2">
                     <Avatar name={searchResult.username} color={searchResult.avatar_color} size="md" />
                     ...
                     <button
                       onClick={handleAddContactAndChat}
                       disabled={addingContact}
                       className="btn-primary rounded-lg px-3 py-1.5 text-xs font-semibold"
                     >
                       {addingContact ? '...' : searchResult.is_contact ? 'Chat' : 'Add Contact'}
                     </button>
                   </div>
                 )}
               </>
              ...
              <>
                {loadingContacts ? (
                  ...
                  <p className="text-center py-6 text-sm" style={{ color: 'var(--subtext)' }}>
                    No contacts found. Use "Add Contact" to find people.
                  </p>
                ) : (
                  filteredContacts.map(user => {
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-colors ${isSelected ? '' : 'hover:bg-[var(--hover)]'}`}
